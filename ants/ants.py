...
class Ant(Insect):
    ...
    def double(self):
        ...
        # BEGIN Problem 12
        if not self.is_doubled and self.damage != 0:
            self.damage *= 2
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        def find_bees(place, distance=0):
            if place.is_hive:
                return None
            elif len(place.bees) > 0 and self.lower_bound <= distance <= self.upper_bound:
                return random_bee(place.bees)
            else:
                return find_bees(place.entrance, distance + 1)
        return find_bees(self.place, 0)
        # END Problem 3 and 4
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        """First, checking if it is chewing, if so, decresement its turns_to_chew().
        Otherwise, eat a random Bee in its place."""
...
class TankAnt(ContainerAnt):
    """TankAnt provides protection to other Ants and deals 1 damage to all bees."""
    name = 'Tank'
    food_cost = 6
    damage = 1
    implemented = True
...
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        while curr_exit:
            if curr_exit.ant:
                curr_exit.ant.double()
                if curr_exit.ant.is_container and curr_exit.ant.ant_contained:
                    curr_exit.ant.ant_contained.double()
    ...
    def remove_from(self, place):
        # BEGIN Problem 12
        """A queen cannot be removed. Attempts to remove a queen should have no effects."""
        return None
        # END Problem 12
